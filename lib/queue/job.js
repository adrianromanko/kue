    ...
    return function(err, ids) {
        ...
        ids.forEach(function(id) {
            exports.get(id, function(err, job) {
                ...
                --pending || fn(null, 'desc' == order ? map(jobs, ids).reverse() : map(jobs, ids));
...
exports.get = function(id, fn) {
    ...
    client.hgetall(client.getKey('job:' + job.id), function(err, hash) {
        ...
        if (!hash.type) {
            exports.removeBadJob(job.id, hash);
...
exports.remove = function(id, cb) {
    ...
    exports.get(id, function(err, job) {
        ...
        if (!job) return cb(new Error('Failed to find job [' + id + '].'));
        job.remove(cb);
    });
};
...
exports.log = function(id, fn) {
    var client = redis.client();
    client.lrange(client.getKey('job:' + id + ':log'), 0, -1, fn);
};
...
Job.prototype.setClient = function(client) {
    this.client = client;
    return this;
};
/**
...
Job.prototype.priority = function(level) {
    ...
    this._priority = null == priorities[level] ? level : priorities[level];
    return this;
};
...
Job.prototype.attempt = function(cb) {
    var self = this,
        client = this.client,
        id = this.id,
    ...
    client.hincrby(key, 'attempts', 1, function(err, attempts) {
        cb(err, Math.max(0, self._max_attempts - attempts), self._attempts, self._max_attempts);
    });
...
Job.prototype.removeState = function(cb) {
    var client = this.client,
        state = this._state;
    if (state && state != '') {
        var multi = client.multi()
...
Job.prototype.state = function(state, fn) {
    ...
    if (this._state && this._state != state) { // delete old _state
        multi
            .zrem(client.getKey('jobs:' + this._state), this.id)
            .zrem(client.getKey('jobs:' + this.type + ':' + this._state), this.id);
    }

    this._state = state;
    multi
        .hset(client.getKey('job:' + this.id), 'state', state)
        .zadd(client.getKey('jobs:' + this._state), this._priority, this.id)
        .zadd(client.getKey('jobs:' + this.type + ':' + state), this._priority, this.id)
        .hset(client.getKey('job:' + this.id), 'updated_at', Date.now());

    if (state === 'inactive') { // put job back into list - used by Worker.getNextJob()
        multi.lpush(client.getKey(this.type + ':jobs'), 1)
    }
...
Job.prototype.save = function(fn) {
    ...
    client.incr(client.getKey('ids'), function(err, id) {
        ...
        self.subscribe(function() {
            ...
            // state
            multi
                .hset(client.getKey('job:' + this.id), 'state', self._state)
                .zadd(client.getKey('jobs:' + self._state), this._priority, this.id)
                .zadd(client.getKey('jobs:' + this.type + ':' + self._state), this._priority, this.id)

            if (self._state === 'inactive') { // increase available jobs, used by Worker#getJob()
                multi.lpush(client.getKey(this.type + ':jobs'), 1)
            }
            ...
            multi.hset(key, 'created_at', Date.now());
            ...
            // delay
            if (self._delay) {
                multi.hset(key, 'delay', self.delay);
            }
            if (self._removeOnComplete) {
                multi.hset(key, 'removeOnComplete', self._removeOnComplete);
            }

            if (self._backoff) {
                if (_.isPlainObject(self._backoff)) {
                    multi.hset(key, 'backoff', JSON.stringify(this._backoff));
                } else {
                    multi.hset(key, 'backoff', this._backoff.toString());
                }
            }
            ...
            // priority
            multi.hset(key, 'priority', this._priority);
            multi.zadd(client.getKey('jobs'), this._priority, this.id);

            // data
            multi.hset(key, 'data', json);

            multi.exec(function(err, data) {
                if (err) return fn(err);
                fn(null, data);
                //self.state(this._state, fn);
            }.bind(this));
...
Job.prototype.update = function(cb) {
    ...
    // delay
    if (this._delay) {
        multi.hset(this.client.getKey('job:' + this.id), 'delay', this._delay);
    }

    if (this._backoff) {
        if (_.isPlainObject(this._backoff)) {
            multi.hset(this.client.getKey('job:' + this.id), 'backoff', JSON.stringify(this._backoff));
        } else {
            multi.hset(this.client.getKey('job:' + this.id), 'backoff', this._backoff.toString());
        }
    }

    // updated timestamp
    multi.hset(this.client.getKey('job:' + this.id), 'updated_at', Date.now());

    // priority
    multi.hset(this.client.getKey('job:' + this.id), 'priority', this._priority);


    multi.zadd(this.client.getKey('jobs'), this._priority, this.id);

    // data
    multi.hset(this.client.getKey('job:' + this.id), 'data', json);
